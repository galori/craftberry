...
After the crafting assertion, a teardown block deactivates both imported packs for **My World** so repeated runs don't leave more and more test add-ons active in the game — this cleanup runs whether the test above passes or fails, and never affects the test's own pass/fail result (a cleanup hiccup is logged as a screenshot, not a failure). It pauses the running world, saves and quits to the world list, reopens **Edit World**, and removes the behavior pack at the top of the Active list (reliably the one this run just activated, since nothing else can interleave an activation mid-test) plus its confirmation dialog ("Just continue", not "Copy and continue" — this is a disposable test world). The matching resource pack is only removed if its name ("Emerald Test Sword Resources") is unambiguous in the currently visible portion of the Active list; Minecraft doesn't insert newly auto-activated resource packs at a predictable rank the way it does manually activated behavior packs, so this step skips (and logs) rather than risk removing an unrelated pack. In practice this means the resource-pack half of cleanup only becomes fully effective once any pre-existing "Emerald Test Sword Resources" duplicates (left over from before this cleanup existed) are cleared once by hand — after that, each run leaves at most one, so the name is always unambiguous. Note: Minecraft's own "required dependency" guard additionally refuses to remove a resource pack still depended on by an active behavior pack, so even a wrong match cannot break another active pack. Deactivating a pack does not remove it from Minecraft's installed pack library — no automatable control for that was found (see `docs/MINECRAFT_DEVICE_AUTOMATION.md`); fully uninstalling remains a manual step.
