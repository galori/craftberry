...
Status as of 2026-08-10. “Automated” means deterministic local XCTest coverage. “Device” requires an exported artifact imported into the exact Minecraft release on a physical iPhone with a clean Content Log.
...
| Material ingot + furnace-smelted sword collection (ingot + sword) | Supported | Typed `minecraft:recipe_furnace` (`tags: ["furnace"]`, string `input` string `output`, `unlock`), shaped sword recipe, 32×32 ingot/sword sprites, localization, packs | Exact assertions pinned to `behavior_pack/recipes/furnace_beef.json` / `furnace_iron_ore.json` `minecraft:recipe_furnace` at `921fafb0` | Yes | Pending | Phase 4d code complete |
| Material ingot + brewing-stand elixir collection (ingot + elixir) | Supported | Typed `minecraft:recipe_brewing_mix` + `minecraft:recipe_brewing_container` (`tags: ["brewing_stand"]`, string `input`/`reagent`/`output`), shapeless ingot recipe, 32×32 ingot/elixir sprites, localization, packs | Exact assertions pinned to `behavior_pack/recipes/brew_awkward_blaze_powder.json` `minecraft:recipe_brewing_mix` and `behavior_pack/recipes/brew_potion_sulphur.json` `minecraft:recipe_brewing_container` at `921fafb0` | Yes | Pending | Phase 4e code complete |
| Material ingot + ore block collection (ingot + ore) | Supported | Typed `minecraft:block` (`destroy_time`/`map_color`/`loot` → ingot + `light_dampening`), `loot_tables/blocks/*.json` → ingot, `terrain_texture.json` + `blocks.json`, 32×32 ore-item + 16×16 ore terrain tile, shapeless ingot + shaped 9→1 ore, localization, packs | Exact assertions pinned to `minecraft:block` + `terrain_texture.json`/`blocks.json` and loot `name: craftberry:*_ingot` at `921fafb0` (ore loot drops ingot, not self) | Yes | Pending | Phase 4f code complete |
| Material seed + crop block + produce collection (seed + crop + produce) | Supported | Typed `minecraft:block` with `states: {"craftberry:growth": {"values": {"min":0,"max":7}}}` + 8 `permutations` (`query.block_state('craftberry:growth') == N`), `loot_tables/blocks/*.json` → produce, `terrain_texture.json` + `blocks.json`, 32×32 seed/produce + 16×16 crop terrain tile, shapeless 4× seed, localization, packs | Exact assertions pinned to `metadata/json_schemas/server/block/1.21.110/States.json` + `Block Permutations.json` at `921fafb0` + vanilla crop states/permutations pattern | Yes | Pending | Phase 4g code complete |
| Material ingot + passive entity collection (ingot + creature) | Supported | Typed `minecraft:entity` (`1.26.10`, creature/passive/health/type_family/breathable/collision/movement/physics/pushable/nameable, adult/baby component_groups, loot/breedable), `minecraft:spawn_rules` (`1.8.0`, animal/grass_block/brightness/weight/herd/biome), `loot_tables/entities/*.json`, `minecraft:client_entity` (`1.10.0`, `entity_alphatest`/`pig` geometry/render controller, spawn_egg), spawn-egg `item_texture.json` + 32×32 spawn-egg sprite, shapeless ingot + shaped spawn egg (egg + 4 ingot), localization, packs | Exact assertions pinned to `behavior_pack/entities/chicken.json`, `spawn_rules/chicken.json`, `resource_pack/entity/chicken.entity.json` at `921fafb0` | Yes | Pending | Phase 4h code complete |
...
| Shapeless, furnace, brewing, smithing transform/trim | Shapeless supported for material ingots; furnace supported via furnace set; brewing supported via brewing set | Brewing: `minecraft:recipe_brewing_mix`/`container` via brewing set; smithing planned | Fire-charge evidence for shapeless; furnace_beef/iron_ore and brew_awkward_blaze_powder/brew_potion_sulphur evidence at `921fafb0` | Yes for shapeless + furnace + brewing | Pending | Phase 2A / 4d / 4e |
| Blocks, ores, crops, material families | Supported for storage block, ore, and crop; features remaining | Supported for storage block, ore (`loot` → ingot), and crop (`states`/`permutations`) | Required per remaining archetype (features) | Planned for features | Pending for features | Phase 3 (ore/crop code complete) |
| Entities, projectiles, spawning, loot, trades | Supported for passive entity; hostile/tameable/rideable/projectile/boss remaining | Supported for passive entity | Required per remaining archetype | Planned for remaining | Pending | Phase 4 |
...
| Material furnace (furnace-smelted ingot + sword) set generation through OpenAI structured intent | Automated |
| Material brewing (brewing-stand elixir) set generation through OpenAI structured intent | Automated |
| Material ore (ingot+ore) set generation through OpenAI structured intent | Automated |
| Material crop (seed+crop+produce) set generation through OpenAI structured intent | Automated |
| Material passive entity (ingot+creature) set generation through OpenAI structured intent | Automated |
...
| 2026-08-09 | Add material furnace sets (furnace-smelted ingot + sword) as the first stable furnace slice | The smallest bounded whole-family that introduces `minecraft:recipe_furnace` while reusing the ingot + shaped-sword + `item_texture.json` pipeline: one furnace recipe (`tags: ["furnace"]`, string `input`/`output`, `unlock`) that smelts the vanilla source into the ingot, plus one shaped sword (2 ingot + stick), 32×32 ingot/sword sprites. Pinned to `furnace_beef` / `furnace_iron_ore` at `921fafb0`. |
| 2026-08-09 | Add material brewing sets (brewing-stand elixir) as the first stable brewing slice | The smallest bounded whole-family that introduces `minecraft:recipe_brewing_mix` + `minecraft:recipe_brewing_container` while reusing the shapeless-ingot + `item_texture.json` pipeline: one shapeless ingot (1..9 vanilla source → ingot), one `brewing_mix` (potion + ingot → elixir, `tags: ["brewing_stand"]`) and one `brewing_container` (elixir + gunpowder → splash_potion, `tags: ["brewing_stand"]`), 32×32 ingot/elixir sprites. Pinned to `brew_awkward_blaze_powder` / `brew_potion_sulphur` at `921fafb0`. |
| 2026-08-10 | Add material ore sets (ingot + ore block that drops ingot) as the first ore slice | The smallest bounded whole-family that introduces `minecraft:block` loot-override + `terrain_texture.json`/`blocks.json`: one shapeless ingot (1..9 vanilla source → ingot), one shaped ore (9 ingot → ore, loot `craftberry:*_ingot`), 32×32 ore-item + 16×16 ore terrain tile. Pinned to `minecraft:block` + `terrain_texture.json`/`blocks.json` at `921fafb0`; feature placement deferred (requires `features/` + `feature_rules/` scatter configs not yet stable without Blockception vendoring). |
| 2026-08-10 | Add material crop sets (seed + crop block with 8 growth stages + produce) as the first crop slice | The smallest bounded whole-family that introduces `minecraft:block` `states`/`permutations` + produce loot: one shapeless 4× seed (1..9 vanilla source → seed), one crop block (`craftberry:growth` 0..7, 8 permutations on `query.block_state('craftberry:growth')`), `loot_tables/blocks/*.json` → produce, 32×32 seed/produce + 16×16 crop terrain. Pinned to `metadata/json_schemas/server/block/1.21.110/States.json` + `Block Permutations.json` at `921fafb0` plus vanilla wheat/crop states pattern. |
